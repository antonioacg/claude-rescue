...
| Script | Scope | Server |
|---|---|---|
| `scripts/validate.sh` | 26 assertions across 11 unit-level scenarios: window UUID minting, `/clear` handling, multi-pane sharing, title debounce, pane-died flush, resurrect cycle, window swap, no-tmux fallback, find-sessions dotfile-cwd encoding, resurrect-save sentinel encoding for empty window_name, active session-id file lifecycle (write on SessionStart, overwrite on in-claude `/resume`, clear on SessionEnd, bulk-clear on resurrect-restore), picker data plane, install dry-run, JSONL validity, source-file tilde scan | isolated `claude-rescue-validate` socket, temp data dir — doesn't touch staging |
