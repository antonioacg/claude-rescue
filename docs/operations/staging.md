...
| Script | Scope | Server |
|---|---|---|
| `scripts/validate.sh` | 31 assertions across 12 unit-level scenarios: window UUID minting, `/clear` handling, multi-pane sharing, title debounce, pane-died flush, resurrect cycle, window swap, no-tmux fallback, find-sessions dotfile-cwd encoding, resurrect-save sentinel encoding for empty window_name, active session-id file lifecycle (write on SessionStart, overwrite on in-claude `/resume`, clear on SessionEnd, bulk-clear on resurrect-restore), resurrect-save snapshot-diff (first save emits nothing, changed title emits one event, unchanged title emits none, sibling resurrect-dirs don't pollute prev-lookup), picker data plane, install dry-run, JSONL validity, source-file tilde scan | isolated `claude-rescue-validate` socket, temp data dir — doesn't touch staging |
| `scripts/validate-hibernation.sh` | 57 assertions across (1) soft hibernation, (2) soft resume, (3) hard escalation with `clr <sid>` pre-fill, (3b/3c) hard marker lifecycle (survives focus-in, cleaned by `cmd_session_start`), (4) fast guard on non-claude panes, (5) orphan arm subshell self-bails on identity mismatch, (6) `arm-sweep` arms every backgrounded claude pane idempotently, (7) `hibernate-resume` preserves the in-flight arm subshell when marker is `mode=hard`, (8) SessionEnd reaps arm.pid + active session file (keeps `@claude-pane-id` as identity bridge), (9) `arm-sweep` voluntary-exit detection cleans state when a pane has `@claude-pane-id` but no longer runs claude | uses staging: teardown + `staging.sh setup` + `staging-fixture.sh` + drives via `tmux run-shell` |
| `scripts/validate-crash-restore.sh` | 31 assertions across four crash-restore paths: (1) soft-saved-as-claude → wrapper auto-resumes session, marker auto-promoted with `hard_source: "crash-promote"`, cleaned by post-resume `session_start`; (2) hard-saved-as-zsh → post-restore-keys subshell prints capture + pre-fills `clr <sid>`, **marker survives** (crash-restore insurance); (3) hard → focus-in → fresh `cl` (new session) → hard again → crash → restore → NEW session_id is what gets pre-filled, not the original; (4) wrapper resolution prefers `$DATA/active/<pane_uuid>` over a stale saved `-r` arg (the in-claude `/resume` staleness regression) | uses staging: rebuilds between scenarios via `bring_up_fresh` |
